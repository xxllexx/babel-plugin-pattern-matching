    ...
    return {
        ...
        visitor: {
            ...
            VariableDeclaration: function VariableDeclaration(path, state) {
                if (state.opts && state.opts.enableByComment && (!path.node.leadingComments || !path.node.leadingComments.some(function (l) {
...
var $$enableComment = '@enable-match';
...
var typePredicates = {
    ...
    ObjectExpression: function ObjectExpression(block, t) {
        if (!block.properties.length) {
            ...
            var keysLength = block.properties.some(function (e) {
                return t.isSpreadProperty(e);
            }) ? block.properties.length - 1 : block.properties.length;
...
var typeArguments = {
    ...
    ArrayExpression: function ArrayExpression(block, t) {
        return block.elements.length ? block.elements.map(function (a) {
            return getTypeArguments((a || { type: 'Skip' }).type)(a, t);
        }) : false;
    },
    BinaryExpression: function BinaryExpression(block, t) {
        return block.operator === '&' ? [t.identifier(block.left.name)].concat(_toConsumableArray(getTypeArguments(block.right.type)(block.right, t))) : null;
    },
...
var getTypeParams = {
    ...
    ArrayExpression: function ArrayExpression(block, t, property) {
    ...
    BinaryExpression: function BinaryExpression(block, t) {
        if (block.operator === '&') {
            var res = getTypedParams(block.right.type)(block.right, t);
            res.elements = [t.stringLiteral($$getAllObject)].concat(_toConsumableArray(res.elements));
            return res;
        }
        return null;
    },
    ObjectProperty: function ObjectProperty(block, t) {
        return t.isIdentifier(block.value) ? t.stringLiteral(block.key.name) : t.objectExpression([t.objectProperty(t.identifier(block.key.name), getTypedParams(block.value.type)(block.value, t, true))]);
    },
    SpreadProperty: function SpreadProperty(block, t) {
        return t.stringLiteral($$getRestParams);
    },
    ObjectExpression: function ObjectExpression(block, t) {
        return t.arrayExpression(block.properties.length ? block.properties.map(function (p) {
